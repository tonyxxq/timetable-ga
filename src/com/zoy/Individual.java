...
public class Individual {
	...
	public Individual(Timetable timetable) {
		...
		// 1 gene for time, 2 for teacher
		int chromosomeLength = numClasses * 2;
		// Create random individual
		int newChromosome[] = new int[chromosomeLength];
		int chromosomeIndex = 0;
		
		Set<Integer> set = timetable.getTeachers().keySet();
		Integer[] xx =  set.toArray(new Integer[] {});
		shuffle(xx);
		for (int teacherId : set) {
			// Add teacher
			newChromosome[chromosomeIndex] = teacherId;
			chromosomeIndex++;
	...
	public void shuffle(Integer[] set) {
